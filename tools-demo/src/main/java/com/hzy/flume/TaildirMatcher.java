...
public class TaildirMatcher {
    ...
    private List<File> getMatchingFilesNoCache2() {
        List<File> result = Lists.newArrayList();
        File root = parentDir;
        List<File> allDirs = getAllDirs(root,Lists.newArrayList());
        for(File f:allDirs){
            try (DirectoryStream<Path> stream = Files.newDirectoryStream(f.toPath(), fileFilter)) {
                for (Path entry : stream) {
                    result.add(entry.toFile());
                }
            } catch (IOException e) {
    ...
    private List<File> getAllDirs(File root, List<File> result) {
        if (!root.isDirectory()) {
            return result;
        } else {
            result.add(root);
